    ...
    try:
        ...
        for task in overdue_file_tasks:
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6;">
                ...
                <p>Yêu cầu nộp file '<b>{task.title}</b>' đã quá hạn vào lúc {task.deadline.strftime('%H:%M %d/%m/%Y')} (giờ UTC).</p>
            """
        ...
        for report in overdue_data_reports:
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6;">
                <p>Chào Quản trị viên,</p>
                <p>Yêu cầu nhập liệu '<b>{report.title}</b>' đã quá hạn vào lúc {report.deadline.strftime('%H:%M %d/%m/%Y')} (giờ UTC).</p>
            """
